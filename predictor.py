...
def main():
    ...
    for race in data_races.keys():
        print("\nModel for race=",race)
        model = construct_model(data_races[race],race)
        models[race]=model
    
    final_preds(models)
    print("Done")

def construct_model(raw,race):
...
def score_model(y_preds,y_actual):
    ...
    accuracy = (np.where(y_preds==y_actual,1,0).sum())/len(y_preds)
    mcc = calc_mcc(y_preds,y_actual)
    return accuracy,mcc
...
def final_preds(models):
    ...
    data,columns = trim_data(predict_df,"test")
